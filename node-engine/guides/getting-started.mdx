...
If no version is specified, your app will be built with current default version (16). To ensure your project is built with a version of your choice, make sure to include your desired settings in "engines" section of your package.json. **Note:** please be aware that only major versions are available - minor and patch versions will be rounded to a given major version.

Depending on chosen major node.js version, environments are built using `node:<version>-slim` images (visit [Docker](https://hub.docker.com/_/node) page for more details)
