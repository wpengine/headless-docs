...
It is possible to query fields that appear on the search config page, this includes ACF fields, ACM fields, tags and categories etc. consider the following query:

```
seats.count:>4 tags.name:Cars hello AND world OR 123 4.56
```

This search string is a combination of search terms and filters. It searches for records that match the following criteria:
