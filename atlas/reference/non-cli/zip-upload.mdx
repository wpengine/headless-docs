...
# Purpose
This functionality allows us to update code for an existing upload-enabled Atlas environment, from a local zip, without using `Atlas CLI` (impossible with Portal). The target local repo must be zipped to upload.
...
###Notes:
CURL HTTP Response will give `200` status, but will not return a response body, if successful. If you would like more detail add the `-i` (light summary) or `-v` (detailed summary) flag to the avove CURL command. You can also check Portal to see the Atlas Environment rebuilding state.
