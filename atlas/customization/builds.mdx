...
This creates one webhook with 3 options: `Rebuild`, `Clean Rebuild` and `Purge Cache`. The `Clean Rebuild` will clear all cached assets, but is otherwise idential to `Rebuild`. The `Purge Cache` will clear the CDN cache for your Atlas domain. Copy one of the webhook URLs to use in your custom workflows:
