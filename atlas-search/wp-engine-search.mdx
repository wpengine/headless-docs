...
WP Engine Search adds debug information to WPGraphQL queries. To enable this, navigate to the WPGraphQL settings page and check the setting "Enable GraphQL Debug Mode".

![WPGraphQL Debug Setting](./images/as-wpgraphql-debug-settings.png)

#### Query Monitoring for non-WPGraphQL Users

WP Engine Search works seamlessly with the [Query Monitor plugin](https://wordpress.org/plugins/query-monitor/) for non-WPGraphQL users (see "Debug Information" above for how to access logs if you are using WPGraphQL).
...
Atlas Search works completely out of the box, no code changes needed. However, some advanced customers prefer to use [WPGraphQL](https://www.wpgraphql.com/docs/introduction) to write custom queries. If this describes you, or is interesting to you, read along. If not, don't worry! Atlas Search will do everything in the above documentation, with no code changes required!

Once your site is set up with an WP Engine Search beta license and the initial content sync has completed, WP Engine Search will take over the default WordPress search functionality and enhance your WordPress search experience.

<em>
  **Note:** If you have other search tools installed on your site, you may need
  to deactivate or uninstall them for WP Engine Search to work properly.
</em>

For advanced users wanting to test search queries, the WPGraphQL plugin is a good place to start. WPGraphQL provides a ‘GraphiQL IDE' in the WordPress dashboard where you can interact with the WPGraphQL Schema in the browser. WP Engine Search overrides the default search functionality for WPGraphQL queries on all WP Engine Search supported WordPress data types.

Below is a screenshot of the WPGraphQL plugin & GraphiQL IDE view, where WP Engine Search has overridden the default WP search behavior, followed by some sample queries:
