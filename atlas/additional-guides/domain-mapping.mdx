...
If your list of custom domains mapped to your Atlas environment contains subdomains (e.g. `my.domain.com`), make sure that a domain with a wildcard (`*.domain.com`) is configured properly on the DNS side, too (so host name is equal to `*` and A record / CNAME is equal to required WP Engine values).
