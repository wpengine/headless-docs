...
3. You have downloaded & activated the [WP-GraphQL](https://wordpress.org/plugins/wp-graphql/) plugin from the WordPress.org plugin repository
  * Note: The WP-GraphQL plugin may already be automatically installed if you used Atlas Blueprints to setup your Atlas app and WordPress environments
...
2. You have received an Atlas Search setup email with the following:
    ...
    + **Do not share this setup email or the Atlas Search credentials**
  

### Plugin Installation
Installing & configuring the Atlas Search plugin enables both the Content Sync & Search Querying functionalities.
