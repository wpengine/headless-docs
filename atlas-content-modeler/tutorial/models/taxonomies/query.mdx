---
slug: /atlas-content-modeler/tutorial/models/taxonomies/query
title: Atlas Content Modeler Taxonomies - Query
description: Learn how to Query GraphQL Taxonomies in Atlas Content Modeler.
---

**Note**: These tutorials assume that you have already [installed the plugin](../../../introduction.mdx).

## Query GraphQL for a Taxonomy

Learn how query taxonomies in GraphQL.

### Steps to Query a Taxonomy in GraphQL

---

### Dependencies

Querying taxonomies requires the installation of a third party plugin called WPGraphQL. Please download and install WPGraphQL following these instructions: [WPGraphQL Installation - Quick Start](https://www.wpgraphql.com/docs/quick-start/). Once you have successfully installed and activated it, you may continue with the steps below for Querying Taxonomies.

1. Click on Content Modeler in the WordPress side menu

   ![Atlas Content Modeler sync](../../../images/acm-sidebar.png)

1. If it's not on Taxonomies, click Taxonomies under Content Modeler in the WordPress side menu

   ![Atlas Content Modeler sync](../../../images/taxonomy-menu.png)

1. You will then see the form to Add a Taxonomy and the current Taxonomies table

   ![Atlas Content Modeler sync](../../../images/taxonomies.png)

1. Fill in the fields to fit your needs

1. Taxonomy Id is auto generated but you can change it to suite your needs

1. Click Create to save the Taxonomy, it will be added to the table

## Feedback

- **Questions/Comments?** [Content Modeler Feedback Form](https://docs.google.com/forms/d/e/1FAIpQLScc2VN-GRSJMz8zVgJLL6kiX3VeV2jkSDnmU1gnuNElEHCEVQ/viewform)
- **GitHub**: [GitHub Repo Feedback](https://github.com/wpengine/atlas-content-modeler/issues/new/choose)
