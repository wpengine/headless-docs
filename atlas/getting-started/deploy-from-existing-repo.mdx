...
Now that you have [chosen to deploy from an existing repository](./blueprint-repository), you can select the repository and deploy it to Atlas. Pushing code changes to the branch of your choice in GitHub will then deploy updates to Atlas automatically.
...
2. Create a `ATLAS_BUILD_SCRIPT` environmental variable in the User Portal, with the command to be run as its value. 
...
After running either `npm run build` or a [custom build command](https://developers.wpengine.com/docs/atlas/customization/builds#custom-build-commands), the platform will start your application using `npm run start` or your [custom start command](../customization/builds#custom-start-commands). The platform expects your application to listen for traffic on one of the following ports: `8080`(default) or `3000`.
...
4. Select `Continue` to proceed to the WP Engine Settings page.
...
4. In order for your WordPress environment to become "headless", Atlas can install the [WP GraphQL](https://wordpress.org/plugins/wp-graphql/), [Atlas Content Modeler](https://wordpress.org/plugins/atlas-content-modeler/), [FaustWP](https://wordpress.org/plugins/faustwp/) and [Atlas Search](https://wordpress.org/plugins/atlas-search/) plugins. It can also automatically set the required environment variables for you so that the FaustWP plugin will work properly out-of-the-box.
