...
This Fuzziness can be toggled, along with modifying this per-word distance (to, let's assume, the maximum distance of 3) in the plugin menu’s ‘Search Config’ view - in this case a search query of "Haaao Waaad" would work as per the previous example, yeilding also  the results for both "Hello" and "World".
