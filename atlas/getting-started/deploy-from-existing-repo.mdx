...
When building the front-end app, WPEngine will use ```npm run build``` by default. If you want to specify a build command, you can do so in one of two ways:

1. adding a "wpe-build" script in the package.json file for your app.
...
3. creating a "ATLAS_BUILD_SCRIPT" environmental variable in the user-portal and adding the command as value. Learn more about environmental variables [here](#env-var).

![ATLAS_BUILD_SCRIPT env variable](./assets/img/env-variables.png)


>If you don't know what command needs to be run to build your app, please refer to the JavaScript framework's documentation.
