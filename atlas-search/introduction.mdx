...
* [Using Atlas Search](#using-atlas-search) - A series of sample GraphQL queries that demonstrate Atlas Search’s functionalities.
    ...
    + [Handling Typos with Fuzziness](#handling-typos-with-fuzziness) - Resolving queries with typos
...
#### Handling Typos with Fuzziness
By default, Atlas Search will attempt to search for a match to every word, with a one-letter-per-word typo tolerence: this means that if you type `Hallo Warld` by accident you will get the results for both `Hello` & `World`. If a word contains more than one typo it will not find such associated words, but it will still search any remaining words in the query unbiasedly.  
