...
1. First lets go and vist our site:

![Atlas Search Plugin Data Sync](./images/as-visit-site.png)

2. Toggle the searh bar on the right hand side:

![Atlas Search Plugin Data Sync](./images/as-visit-site-search-bar.png)

3. Then perform a search query i.e. **"hello"**, Once the search is executed we are redirected to the active themes search results page:
...
Once query monitor is activated lets visit the site and run the **"hello"** search again from steps **1**, **2** and **3** above. 

![Atlas Search Plugin Data Sync](./images/as-query-monitor-hover.png)

With query monitor enabled we get access to a new admin bar item, hovering over it reveals a couple of items that relate to how the page was loaded, we are interested in the **Logs** section.


![Atlas Search Plugin Data Sync](./images/as-query-monitor-navigate-to-logs.png)

Once we click the logs section a new window will open from the bottom of the screen where we will find a log that states the search was successful, these logs will also indicate if there was an issue with the search request being performed
